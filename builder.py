...
# УВАГА: ключі МАЮТЬ точно збігатися з рядками, що повертає op_display_name() у parser.py
_OP_TO_COL = {
    'ПрВ (Прихід)':                'ПрВ',
    'Кнк (Продаж)':                'Кнк',
    'Воз (Повернення)':            'Кнк',  # повернення по чеку: зменшує Кнк (знакова сума)
    'ПрИ (Переміщення)':           'ПрИ',
...
# Операції, що є ПРИХОДОМ у звіті «По документах»
_OP_IS_PRYHID = {
    'ПрВ (Прихід)',              # Прямий прихід
    'Ппт (Переміщення Прихід)',  # Ппт з іншого магазину — приходить до нас
    'Воз (Повернення)',          # Повернення по чеку — товар повертається на склад
}
...
def _agg_cols(df_slice):
    ...
    Правила знаків:
      - ПрВ, СпП → абсолютне значення (abs), завжди ≥ 0
      - Кнк       → знакова сума: Кнк (Продаж) зменшує, Воз (Повернення) збільшує;
                    відображається як abs у заголовку але внутрішньо знакова
    ...
    for op, col in _OP_TO_COL.items():
        ...
        if col in ('ПрВ', 'СпП'):
            ...
            # Кнк, ПрИ, Апс — знакові суми
            # Кнк(Продаж) дає від'ємний qty, Воз(Повернення) — позитивний;
            # сума вийде «чисті продажі» = продажі мінус повернення
            totals[col] += s
...
def build_document_rows(ops_df, prices):
    ...
    Прихід/Розхід визначається по назві операції, а НЕ по знаку qty:
      ПрВ, Ппт(Прихід), Воз          → в колонку Прихід
